...
def upload():
    ...
    try:
        ...
        return rest_api_response(success=False, message="File upload failed", status_code=500)
    ...
    try:
        ...
        db.session.rollback()
        return rest_api_response(success=False, message="Failed to save file record", status_code=500)

    try:
        signed_url = get_presigned_url(s3_key)
    except Exception:
        return rest_api_response(success=False, message="Failed to generate URL", status_code=500)
...
def get_url(media_id: str):
    ...
    try:
        ...
        return rest_api_response(success=False, message="Invalid media ID", status_code=400)
