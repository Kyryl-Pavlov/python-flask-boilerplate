...
def upload():
    if "file" not in request.files:
        return rest_api_response(
            success=False, message="No file provided", status_code=400
        )

    file = request.files["file"]
    if not file.filename:
        return rest_api_response(
            success=False, message="Empty filename", status_code=400
        )
    ...
    try:
        ...
        return rest_api_response(
            success=False, message="File upload failed", status_code=500, exc=e
        )
    ...
    try:
        ...
        db.session.rollback()
        return rest_api_response(
            success=False, message="Failed to save file record", status_code=500, exc=e
        )
    ...
    try:
        ...
        return rest_api_response(
            success=False, message="Failed to generate URL", status_code=500, exc=e
        )
...
def get_url(media_id: str):
    ...
    try:
        ...
        return rest_api_response(
            success=False, message="Invalid media ID", status_code=400, exc=e
        )
